...
export default function BookList() {
  ...
  const loadBooks = useCallback(
    async (append = false) => {
      ...
      try {
        ...
        const newBooks = Array.isArray(response?.content) ? response.content : [];
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      {/* No More Books */}
      {!hasMore && books.length > 0 && <div className="text-center py-8 text-gray-500">No more books to load</div>}

      {/* Empty State */}
      {!isLoading && books.length === 0 && <div className="text-center py-12 text-gray-500 text-lg">No books found</div>}
    </div>
  );
}